...
服务器需要处理两类事件：<br/>
- 文件事件，文件事件是服务器对客户端连接的抽象<br/>
- 时间时间，周期性的运行某个函数

**11.1 serverCron函数做的工作**

- 更新服务器的各类统计信息，比如时间、内存占用、数据库占用情况。
- 清理数据库中过期的键值对。
- 关闭和清理连接失效的客户端。
- 尝试进行AOF和RDB持久化操作。
- 如果服务器是主从模式，那么从服务器定期同步。
- 如果处于集群模式，对集群进行定期同步和连接测试。


**12. 客户端**

通过使用由IO多路复用技术实现的文件事件处理器，redis服务器使用单进程单线程方式处理命令请求，并与多个客户端进行网络通信。

**12.1 客户端的类型**

- 伪客户端：用来执行AOF文件或者LUA脚本。
- 普通客户端：用来处理网络通信。

**12.2 客户端的关闭**

一个普通的客户端可以因为多种原因被关闭：<br/>
- 如果客户端进程退出或被杀死，那么客户端和服务器之间的网络连接将被关闭，从而造成客户端被关闭。<br/>
- 如果客户端向服务器发送了不符合协议格式的命令请求，那么这个客户端也会被服务器关闭。<br/>
- 客户端成为了CLIENT KILL的目标。<br/>
- 如果用户为服务器设置了timeout配置选项，那么当客户端的空转时间超过timeout选项设置的值时，客户端将被关闭。<br/>
- 如果客户端发送的命令请求的大小超过了输入缓冲区限制的大小（默认为1GB）那么客户端将被服务器关闭。<br/>
- 如果客户端的命令回复大小超出了输出缓冲区的限制大小，那么客户端将会被服务器关闭。<br/>

客户端通过两种模式限制客户端输出缓冲区的大小：<br/>
- 硬性限制：如果输出缓冲区超过了硬性限制的大小，那么服务器立即关闭客户端。<br/>
- 软性限制：如果客户端超过了软性限制的大小并且持续时间超过阈值，那么客户端将会被关闭。<br/>
...
**13.1 命令请求的执行过程**<br/>
1. 客户端向服务器发送命令请求；
2. 服务器接收处理客户端的的命令请求，进行相应的数据库操作，并产生回复OK；
3. 服务器将OK回复给客户端；
4. 客户端接受服务器返回的命令恢复在客户度展示。

**13.2 服务器的初始化**<br/>
服务器初始化分两步进行：<br/>
- initServerConfig函数负责初始化一般属性。<br/>
- initServer函数负责初始化数据结构。<br/>
